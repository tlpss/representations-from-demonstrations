...
class Encoder(nn.Module):
    def __init__(self, num_input_channels: int, image_size: int, n_channels: int, latent_dim: int):
        """
        Inputs:
    ...
    def validation_step(self, batch, batch_idx):
        if batch_idx == 0:
            ...
            self.logger.log_image(
                key="validation", images=[grid], caption=["top: source, middle: reconstructions, bottom: target"]
            )
        loss = self.shared_step(batch, batch_idx)
